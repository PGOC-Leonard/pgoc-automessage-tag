...
const QuickMessageWidget = ({ setMessageData }) => {
  const customStyles = {
    control: (base) => ({
      ...base,
      width: "80px",
  ...
  const handleInputChange = (index, event) => {
    ...
    // Dynamically set message_title based on text_message
    if (name === "text_message") {
      updatedBatches[index]["message_title"] = value.slice(0, 20); // First 20 characters as title
    }
      ...
      <div className="bg-gray-100 p-4 rounded-md shadow-sm mb-4 w-full h-[350px] overflow-y-auto">
        {messageBatches.length === 0 ? (
          ...
          messageBatches.map((batch, index) => (
            ...
            >
              <div className="mb-2">
                <label className="label">Shorcut Message:</label>
                <textarea
                  name="text_message"
                  value={batch.text_message}
                  onChange={(e) => handleInputChange(index, e)}
                  className="mt-1 p-2 w-full border border-gray-300 rounded-md text-sm shadow-md focus:outline-none focus:ring-2 focus:ring-[#d32f2f] h-10 font-montserrat resize-none"
                />
              </div>
              <div className="mb-2">
                <div>
